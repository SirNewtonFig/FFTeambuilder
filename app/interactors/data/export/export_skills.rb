...
class Data::Export::ExportSkills < ActiveInteractor::Base
  ...
  def perform
    ...
    Skill.joins(:job).merge(Job.generic).order(Arel.sql "jobs.id, skills.skill_type, skills.data ->> 'memgen_id' desc").includes(:job).each do |skill|
      row = [
        skill.job.name,
        skill_name(skill),
