...
class Character::Memgen < ActiveInteractor::Base
  ...
  private
    ...
    def serialize_secondary!
      block << (SECONDARIES[character.secondary&.skillset] || '00')
    end
    ...
    def serialize_movement!
      support = (MOVEMENTS[character.movement&.name] || '0000')
    ...
    def serialize_rhand_weapon!
      if character.two_hands_engaged?
        block << WEAPONS[character.weapon.name]
      elsif character.rhand&.weapon?
        block << WEAPONS[character.rhand.name]
      else
        pad!('ff', 1)
      end
    end
    ...
    def serialize_rhand_shield!
      if character.rhand&.shield?
        block << SHIELDS[character.rhand.name]
      else
        pad!('ff', 1)
      end
    end
    ...
    def serialize_lhand_weapon!
      if character.two_hands_engaged? || !character.lhand&.weapon?
        ...
        block << WEAPONS[character.lhand.name]
      end
    end

    def serialize_lhand_shield!
      if character.lhand&.shield?
        block << SHIELDS[character.lhand.name]
      else
        pad!('ff', 1)
      end
    end
    ...
    def serialize_hp!
      hp = solve_for_x('hp', character.hp).to_s(16)

      block << little_endian(hp, 3)
    end

    def serialize_mp!
      mp = solve_for_x('mp', character.mp).to_s(16)
      
      block << little_endian(mp, 3)
    end

    def serialize_sp!
      sp = solve_for_x('sp', character.speed).to_s(16)
      
      block << little_endian(sp, 3)
    end

    def serialize_pa!
      pa = solve_for_x('pa', character.attack).to_s(16)
      
      block << little_endian(pa, 3)
    end

    def serialize_ma!
      ma = solve_for_x('ma', character.magic).to_s(16)
      
      block << little_endian(ma, 3)
    end

    def solve_for_x(stat, val)
      (val * RAW_CONST / character.job_data["#{stat}_mult"].to_i).to_i
    end
